...
class HeaderLayout(object):
    ...
    def _trim_line(self, line, model):
        fields = model.get_fields()
        trim_line = []
        for f in fields:
            col_name = f[1].match
            col_index = self.headers.index(col_name)
            trim_line.append(line[col_index])
        return trim_line

    def _validate_headers(self, model):
        fields = model.get_fields()
        for f in fields:
            if f[1].match not in self.headers:
                raise ImproperlyConfigured("Missing header %s. Please check file." % f[1].match)
        return

    def process_line(self, lines, line, model, delimiter):
        if self.line_no == 0:
            self.headers = line
            self._validate_headers(model)
            self.line_no += 1
        else:
            value = model(data=self._trim_line(line, model),
                          delimiter=delimiter)
            lines.append(value)
            return value
...
class CsvImporter(object):
    ...
    def process_line(self, data, line, lines, line_number, model):
        ...
        line_label = line_number + (1 if self.csvModel.has_header() else 0)
        value = None
        try:
            ...
            return False, 'Line # %d. Foreign Key error: %s' % (line_label, e.message)
        except ValueError as e:
            if line_number == 0 and self.csvModel.has_header():
                pass
            else:
                return False, 'Line # %d. Data error: %s %s %s' % (line_label, e.message)
