...
def get_company_name(company_id):
    company_names = dict((entry['id'], entry['name']) for entry in gl_api.companies())
    try:
...
def order_paths(path_list):
    ACCT_LIST = dict((a['id'], a['path']) for a in accountifie.gl.apiv1.accounts())
